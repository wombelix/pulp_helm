...
from pulp_helm.app.models import HelmContent, HelmRemote
...
def synchronize(remote_pk, repository_pk, mirror):
    ...
    """
    remote = HelmRemote.objects.get(pk=remote_pk)
    ...
    first_stage = HelmFirstStage(remote, deferred_download)
    DeclarativeVersion(first_stage, repository, mirror=mirror).create()


class HelmFirstStage(Stage):
    """
    The first stage of a pulp_helm sync pipeline.
    """
    ...
    async def run(self):
        ...
        for entry in self.read_my_metadata_file_somehow(result.path):
            unit = HelmContent(entry)  # make the content unit in memory-only
    ...
    def read_my_metadata_file_somehow(self, path):
        """
        Parse the metadata for helm Content type.
