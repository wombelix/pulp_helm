"""Tests that sync helm plugin repositories."""
import unittest
...
from pulp_helm.tests.functional.constants import (
    HELM_FIXTURE_SUMMARY,
    HELM_INVALID_FIXTURE_URL,
)
...
class BasicSyncTestCase(unittest.TestCase):
    """Sync a repository with the helm plugin."""

    @classmethod
    def setUpClass(cls):
        """Create class-wide variables."""
        cls.cfg = config.get_config()
        cls.client = gen_helm_client()

    def test_sync(self):
        """Sync repositories with the helm plugin.
        ...
        8. Assert that the same number of are present and that no units were
           added.
        """
        repo_api = RepositoriesHelmApi(self.client)
        remote_api = RemotesHelmApi(self.client)
        ...
        self.assertDictEqual(get_content_summary(repo.to_dict()), HELM_FIXTURE_SUMMARY)
        self.assertDictEqual(get_added_content_summary(repo.to_dict()), HELM_FIXTURE_SUMMARY)
...
class SyncInvalidTestCase(unittest.TestCase):
    ...
    @unittest.skip("FIXME: Plugin writer action required.")
    def test_invalid_helm_content(self):
        ...
        """
        with self.assertRaises(PulpTaskError) as cm:
            task = self.do_test(HELM_INVALID_FIXTURE_URL)
        task = cm.exception.task.to_dict()
