...
from pulp_helm.app.serializers import HelmContentSerializer
from pulp_helm.app.models import HelmContent

from pulpcore.plugin.models import Artifact


# Fill data with sufficient information to create HelmContent
# Provide sufficient parameters to create the HelmContent object
# Depending on the base class of the serializer, provide either "_artifact" or "_artifacts"
@unittest.skip("FIXME: plugin writer action required")
class TestHelmContentSerializer(TestCase):
    """Test HelmContentSerializer."""

    def setUp(self):
        """Set up the HelmContentSerializer tests."""
    ...
    def test_valid_data(self):
        """Test that the HelmContentSerializer accepts valid data."""
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = HelmContentSerializer(data=data)
        self.assertTrue(serializer.is_valid())

    def test_duplicate_data(self):
        """Test that the HelmContentSerializer does not accept data."""
        HelmContent.objects.create(artifact=self.artifact)
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = HelmContentSerializer(data=data)
